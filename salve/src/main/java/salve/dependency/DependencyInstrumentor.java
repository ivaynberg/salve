...
public class DependencyInstrumentor implements salve.Instrumentor {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * BytecodePool classPath = new BytecodePool(); classPath.addLoader(new
		 * ClassLoaderLoader(loader)); byte[] bytecode =
		 * classPath.loadBytecode("salve/dependency/Bean"); ClassReader reader =
		 * new ClassReader(bytecode); DependencyAnalyzer locator = new
		 * DependencyAnalyzer(classPath); ClassWriter writer = new
		 * ClassWriter(ClassWriter.COMPUTE_FRAMES); ClassAdapter adapter = new
		 * DependencyInstrumentorAdapter(writer, locator);
		 * reader.accept(adapter, 0);
		 * 
		 * classPath.save("salve/dependency/Bean", writer.toByteArray());
		 * 
		 * FileOutputStream out = new FileOutputStream(
		 * "target/test-classes/Test.class"); final byte[] bytes =
		 * writer.toByteArray(); out.write(writer.toByteArray()); out.close();
		 * 
		 * Class tb = classPath.loadClass("salve/dependency/Bean");
		 * 
		 * Object b = tb.newInstance();
		 * System.out.println(b.getClass().getName()); Bean bb = (Bean) b;
		 * bb.method1(); /* reader.accept(new TraceClassVisitor(new
