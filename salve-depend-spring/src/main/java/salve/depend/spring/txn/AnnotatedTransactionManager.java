...
/**
 * use this transaction manager type to support the famous @Transactional
 * attribute on methods
 * <p/>
 * it will create the required AnnotationTransactionAttributeSource and
 * TransactionInterceptor automatically.
 * <p/>
 * so there is no need to declare them in our spring xml or guice module or
 * whatever.
 * 
 * @author Peter Ertl
 * @see AnnotationTransactionAttributeSource
 * @see TransactionInterceptor
 * @deprecated see {@link SpringTransactionManager} I think this can now be
 *             deprecated in favor of the old {@link SpringTransactionManager}
 *             which has been revamped
 */
public class AnnotatedTransactionManager implements TransactionManager {
	// transaction wrapper (to get access to all this sneaky and hidden methods
	// of spring)
	 ...
	 */
	public AnnotatedTransactionManager(
			final PlatformTransactionManager platformTransactionManager) {
		if (platformTransactionManager == null)
			throw new IllegalArgumentException(
					"platformTransactionManager can not be null");
	...
	/**
	 * this wrapper is required to gain access to the transaction control
	 * methods of the spring class.
	 * <p/>
	 * these are initially protected so we override and delegate them to get
	 * access within the salve transaction manager.
	 */
	private static final class TransactionWrapper extends
		...
		/**
		 * this method delegates to the corresponding protected method which
		 * will clean up an transaction
		 * 
		 * @param transactionInfo
		 *            type-less transaction info (because the type itself is
		 *            hidden in spring)
		 */
		 ...
		 */
		public TransactionInfo start(
				final TransactionAttribute transactionAttribute,
				final String transactionName) {
			return createTransactionIfNecessary(transactionAttribute,
					transactionName);
		}
		...
		/**
		 * this method delegates to the corresponding protected method which
		 * will finish the transaction on errors
		 * 
		 * @param transactionInfo
		 *            type-less transaction info (because the type itself is
		 *            hidden in spring)
		 * @param ex
		 *            error that interrupted the transaction
		 */
